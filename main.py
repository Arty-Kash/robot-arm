...
def setup_meshcat_robot():
    ...
    for data in visual_data:
        ...
        if geom_type == p.GEOM_BOX:
            obj = meshcat.geometry.Box(size)
            geom_transform = tf.identity_matrix()
        elif geom_type == p.GEOM_CYLINDER:
            obj = meshcat.geometry.Cylinder(size[0], size[1])
...
async def simulation_loop():
    ...
    # 開始直後に一度だけ物体を作成
    setup_meshcat_robot()
    print("Robot objects initialized in Meshcat.")
...
async def message_stream(request: Request):
    async def event_generator():
        while True:
            ...
            yield json.dumps({"step": current_step, "reward": current_reward, "status": "Running"})
...
if __name__ == "__main__":
    import uvicorn
    # 修正したURLログ
    url = vis.url().replace("127.0.0.1", "localhost")
    print(f"\n--- Meshcat URL: {url}static/ --- \n")
